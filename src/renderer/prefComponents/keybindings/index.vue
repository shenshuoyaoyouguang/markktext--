  ...
  <div class="pref-keybindings">
    ...
    <section class="keybindings">
      <div class="text">
        ...
        <a class="link" @click="openKeybindingWiki">{{ $t('settings.keybindings.online') }}</a>.
      </div>
      <el-table
        :data="keybindingList"
        style="width: 100%"
      >
        <el-table-column prop="description" :label="$t('settings.keybindings.description')">
        </el-table-column>
        <el-table-column prop="accelerator" :label="$t('settings.keybindings.keyCombination')" width="220">
        </el-table-column>
        <el-table-column fixed="right" :label="$t('settings.keybindings.options')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleEditClick(scope.$index, scope.row)" type="text" size="small" :title="$t('settings.keybindings.edit')">
              <i class="el-icon-edit"></i>
            </el-button>
            <el-button @click="handleResetClick(scope.$index, scope.row)" type="text" size="small" :title="$t('settings.keybindings.reset')">
              <i class="el-icon-refresh-right"></i>
            </el-button>
            <el-button @click="handleUnbindClick(scope.$index, scope.row)" type="text" size="small" :title="$t('settings.keybindings.unbind')">
    ...
    <section class="footer">
      <separator></separator>
      <el-button size="medium" @click="saveKeybindings">{{ $t('settings.keybindings.save') }}</el-button>
      <el-button size="medium" @click="restoreDefaults">{{ $t('settings.keybindings.restoreDefaults') }}</el-button>
    </section>
    <section v-if="showDebugTools" class="keyboard-debug">
      <separator></separator>
      <div><strong>{{ $t('settings.keybindings.debugOptions') }}:</strong></div>
      <el-button size="medium" @click="dumpKeyboardInformation">{{ $t('settings.keybindings.dumpKeyboard') }}</el-button>
    </section>
    <key-input-dialog
...
export default {
  ...
  methods: {
      ...
      if (this.keybindingConfigurator && this.keybindingList.length > 0) {
        this.keybindingConfigurator.save()
          .then(success => {
            if (!success) {
              notice.notify({
                title: this.$t('settings.keybindings.errors.failedToSave'),
                type: 'error',
                message: this.$t('settings.keybindings.errors.unexpectedError')
              })
            }
          })
    ...
    handleDuplicateShortcut (id, accelerator) {
      notice.notify({
        title: this.$t('settings.keybindings.errors.shortcutInUse'),
        type: 'warning',
        message: this.$t('settings.keybindings.errors.shortcutInUseMessage', { accelerator })
      })
    },
