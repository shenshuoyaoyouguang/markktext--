...
class FileEncodingCommand {
  ...
  get description () {
    return t('commands.misc.changeEncoding')
  }
  ...
  run = async () => {
    ...
    if (isBom) {
      ...
      this.subcommands.push({
        id: `${encoding}-bom`,
        description: `${getEncodingName(encodingObj)}${t('commands.misc.currentSuffix')}`
      })
    }
    ...
    for (const [key, value] of Object.entries(ENCODING_NAME_MAP)) {
      ...
      const item = {
        id: key,
        description: isTabEncoding ? `${value}${t('commands.misc.currentSuffix')}` : value
      }
      if (isTabEncoding) {
